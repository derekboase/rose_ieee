...
class RandomTrajectory:
    ...
    def trajectory_calculator(self):
        ...
        for t in _time:
            ...
            self.j4_traj.append(np.pi/2.0*np.sin(1.0*np.pi*t/self.end_time) - 3.0*np.pi/4.0)
    ...
    def signal_update(self):
        ...
        # For k = 0 
        _k = 0
        phi_k_joint1 = PHI_INIT
        phi_k_joint2 = PHI_INIT
        phi_k_joint3 = PHI_INIT
        phi_k_joint4 = -PHI_INIT
        ...
        # Phi updates
        phi_k_joint1 = self.phi_estimator(phi_k_joint1, delta_u_1, delta_y_1)
        phi_k_joint2 = self.phi_estimator(phi_k_joint2, delta_u_2, delta_y_2)
        phi_k_joint3 = self.phi_estimator(phi_k_joint3, delta_u_3, delta_y_3)
        phi_k_joint4 = self.phi_estimator(phi_k_joint4, delta_u_4, delta_y_4)
        ...
        plt.subplot(2, 2, 1)
        plt.plot(time_plt, self.j1_traj[:-1])
        plt.plot(time_plt, y1)

        plt.subplot(2, 2, 2)
        plt.plot(time_plt, self.j2_traj[:-1])
        plt.plot(time_plt, y2)

        plt.subplot(2, 2, 3)
        plt.plot(time_plt, self.j3_traj[:-1])
        plt.plot(time_plt, y3)

        plt.subplot(2, 2, 4)
        plt.plot(time_plt, self.j4_traj[:-1])
        plt.plot(time_plt, y4)
        plt.show()
